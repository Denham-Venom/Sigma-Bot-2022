...
public class Swerve extends SubsystemBase {
    ...
    public SwerveDrive sDrive = new SwerveDrive(mSwerveMods, gyro, swerveOdometry, limelight);
    ...
    public void periodic() {
        swerveOdometry.update(sDrive.getYaw(), sDrive.getStates());  
        ...
        if(limelight.hasTarget()) {
            ...
            thetaOut = thetaController.calculate(sDrive.getAngleToTargetRel().getRadians());
            swerveReady.setBoolean(false);
        }
        ...
        if(Constants.Swerve.coastOnDisable){
            if (DriverStation.isEnabled() && currentNeutral == 1){
                sDrive.setNeutral(NeutralMode.Brake);
                currentNeutral = 0;
            }
            else if (DriverStation.isDisabled() && currentNeutral == 0){
                sDrive.setNeutral(NeutralMode.Coast);
                currentNeutral = 1;
            }
        }
